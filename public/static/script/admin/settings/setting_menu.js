...
requirejs(['jquery', 'util', 'fancyBox', 'datatables', 'Spinner'], function ($, util, fancyBox, dataTables, Spinner) {
     ...
     * 加载菜单数据
     */
    $('#menuX').load('/admin/Settings/ajaxGetWechatMenu/', function () {
        Spinner.stop();
        fnBtnListen();
        if ($('.menutop').length > 0) {
    ...
    $('#menu_savebtn').on('click', function () {
        ...
        if (curMenu.hasClass('menutop') && MenuHasChild) {
            ...
            if ($('#rad1').get(0).checked) {
                ...
                // [HttpPost]
                $.post('/admin/MainAjax/bindMenu/', postData, function (k) {
                    if (k > 0) {
    ...
    $('#menu_pubbtn').on('click', function () {
        ...
        util.loading();
        // httpPost
        $.post('/admin/MainAjax/ajaxSetWechatMenu/', {

requirejs(['jquery', 'util', 'fancyBox', 'ztree_loader', 'Spinner'], function ($, util, fancyBox, treeLoader, Spinner) {
    ...
    treeLoader.setting.callback.onClick = function (event, treeId, treeNode) {
        if (treeNode.dataId > 0) {
            ...
            $('#iframe_altercat').attr('src', '../Mainpage/alter_category/id/' + treeNode.dataId);
        }
    };

    // 初始化目录树
    treeLoader.init('#_ztree', '../Product/ajaxGetCategroys/r=' + (new Date()).getTime(), function () {
        $('.fix_bottom').show();
    });
    ...
    fnFancyBox('#add_category_btn', function () {
        ...
        $('#add_cate_btn').click(function () {
            ...
            if (catname !== '') {
                $.post('/admin/Product/ajaxAddCategroy/', {
                    catname: catname,
                    pid: pid
                }, function (r) {
                    if (parseInt(r) > 0) {
